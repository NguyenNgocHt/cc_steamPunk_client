...
export class AudioWebManager extends BaseAudio {
  ...
  playBGM(name: string, fade: boolean = false) {
    ...
    if (data && data.howl) {
      ...
      if (!this.isMutingMusic) {
        ...
        if (fade) {
          data.howl.volume(0);
          data.howl.fade(0, this._musicVolume, BaseAudio.FADE_DURATION * 1000);
        } else {
  ...
  pauseBGM(fade = true) {
    ...
    if (this._webBgm && this._webBgm.howl.playing()) {
      if (!fade) {
        ...
        this._webBgm.howl.fade(this.musicVolume, 0, BaseAudio.FADE_DURATION * 1000);
  ...
  resumeBGM(fade = true) {
    ...
    if (this._webBgm && !this._webBgm.howl.playing()) {
      if (!fade) {
        ...
        this._webBgm.howl.play();
        this._webBgm.howl.fade(0, this._musicVolume, BaseAudio.FADE_DURATION * 1000);
      }
    }
  }
  ...
  playClip(name: string, loop: boolean = false, resumeBGM: boolean = true, callback: VoidFunction | null = null) {
    ...
    if (data && data.howl && !data.howl.playing()) {
      ...
      !loop &&
        this._webClip.howl.once("end", (sId) => {
  ...
  stopClip(resumeBGM: boolean = true, callback: VoidFunction | null = null) {
    ...
    if (this._webClip && this._webClip.howl && this._webClip.howl.playing()) {
      this._webClip.howl.fade(this._effectVolume, 0, BaseAudio.FADE_DURATION * 1000);
  ...
  resumeClip(fade: boolean = false) {
    ...
    if (this._webClip && this._webClip.howl && !this._webClip.howl.playing()) {
      if (!fade) {
        ...
        this._webClip.howl.fade(0, this._effectVolume, BaseAudio.FADE_DURATION * 1000);
        this._webClip.howl.once("fade", (sId) => {
          this._webClip.howl.pause();
        });
      }
  ...
  stopEffect(sfxId: string, fade: boolean = false): boolean {
    ...
    if (data && data.howl && data.howl.playing(parseInt(sfxId))) {
      if (!fade) {
        ...
        data.howl.fade(this._effectVolume, 0, BaseAudio.FADE_DURATION * 1000, parseInt(sfxId));
  ...
  stopEffectByName(name: string, fade: boolean = false): boolean {
    ...
    if (data && data.howl && data.howl.playing()) {
      if (!fade) {
        ...
        data.howl.fade(this._effectVolume, 0, BaseAudio.FADE_DURATION * 1000);
        data.howl.once("fade", (sId) => {
          data.howl.stop();
        });
      }
  ...
  resumeEffect(sfxId: string, fade: boolean = false) {
    ...
    if (data && data.howl && !data.howl.playing(parseInt(sfxId))) {
      if (!fade) {
        ...
        data.howl.play(parseInt(sfxId));
        data.howl.fade(0, this._effectVolume, BaseAudio.FADE_DURATION * 1000, parseInt(sfxId));
      }
    }
  }
