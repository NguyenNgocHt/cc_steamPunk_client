...
export default class ScreenManager extends cc.Component {
  ...
  public static get instance(): ScreenManager {
    if (!ScreenManager._instance) {
      ScreenManager._instance = cc.director.getScene()!.getComponentInChildren(ScreenManager)!;
    }
    return ScreenManager._instance;
  }

  // public static set instance(value) {
  //     ScreenManager._instance = value;
  // }
  ...
  onLoad() {
    ...
    // ScreenManager.instance = this._canvas.getComponent("ScreenManager")! as ScreenManager;
    if (cc.sys.isBrowser) {
  ...
  public changeCanvasBackground(bgrSprite: cc.SpriteFrame) {
    if (bgrSprite && ScreenManager.instance.layerBackground) {
  ...
  onDestroy() {
    cc.log("[ScreenManager] onDestroy !");
  ...
  public get curScreen(): BaseScreen {
    return this._screenStack[this._screenStack.length - 1];
  }

  private _popUpStack: Array<BasePopup> = [];

  public get screenStack(): Array<BaseScreen> {
    return this._screenStack;
  }
  private _screenStack: Array<BaseScreen> = [];
  ...
  initWithRootScreen(screen: cc.Prefab, callback?: Function | null) {
    ...
    this.loadScreen(screen, function (nextScreen: BaseScreen) {
  ...
  pushScreen(screen: cc.Prefab, callback?: Function | null, animated: boolean = false, zIndex: number = VD_ZINDEX.SCREEN) {
    ...
    if (this._screenStack.length != 0) {
      ...
      this._canvas.emit(ControlEvent.CanvasCancel);
  ...
  popScreen(animated: boolean = false, showWaiting: boolean = true): BaseScreen | null {
  ...
  public findScreen(classNameOrComponent: any): BaseScreen | null {
  ...
  public popToScreen(classNameOrComponent: any, animated: boolean = false): BaseScreen | null {
    let screenIdx = -1;
    for (let i = this._screenStack.length - 1; i >= 0; i--) {
  ...
  public popToScreenAtIndex(screenIdx: number, animated: boolean = false, showWaiting: boolean = true): BaseScreen | null {
    if (this._animating || this._screenStack.length <= 1) {
      return null;
    }
    ...
    this.scheduleOnce(function () {
      ...
      if (self._screenStack.length > screenIdx + 1) {
        for (let i = screenIdx + 1; i < self._screenStack.length; i++) {
          ...
          if (screen.node.parent) {
            screen.node.emit(ControlEvent.ScreenDidPop);
            screen.node.removeFromParent();
            screen.node.destroy();
          } else {
            screen.node.destroy();
  ...
  public replaceScreenAtIndex(
    screen: cc.Prefab,
    screenIdx: number,
    callback?: Function | null,
    animated: boolean = false,
    zIndex: number = VD_ZINDEX.SCREEN,
  ...
  //         this.loadScreen(screen, function (node: BaseScreen) {
  ...
  getCurrentPopup(): BasePopup | null {
  ...
  removePopupNode(node: cc.Node, animated: boolean = true) {
    if (node.getComponent(BasePopup)) {
      this.removePopup(node.getComponent(BasePopup)!, animated);
    }
  }

  removePopup(popup: BasePopup, animated: boolean = true) {
    ...
    if (localIndex >= 0) {
      if (localIndex == this._popUpStack.length - 1) {
        ...
        popup.node.emit(ControlEvent.PopupWillDisappear);
        popup.node.emit(ControlEvent.PopupDidDisappear);
  ...
  showPopupFromNode(nodePopup: cc.Node, callback: Function | null = null, hideWhenTouchOnBackground: boolean = true, animated: boolean = true, showBackgroundLayer: boolean = true) {
    ...
    if (this._popUpStack.length == 0) {
      ...
      if (prevPopup.name === VD_POPUP_NOTIFY_NAME && nodePopup.name === VD_POPUP_NOTIFY_NAME) {
        ...
        // console.log(`ScreenManager - showPopupFromNode - currentPopup: ${prevPopup.name}`);
      }
    }
    this.popupBackground.setZIndex(VD_ZINDEX.POPUP + this._popUpStack.length * 2);

    // console.log(`ScreenManager - showPopupFromNode - newPopup: ${nodePopup.name}`);
    let popComp = nodePopup.getComponent(BasePopup) as BasePopup;
    if (!popComp) {
      popComp = nodePopup.addComponent(BasePopup) as BasePopup;
    }
    ...
    this.scheduleOnce(function () {
      self._canvas.emit(ControlEvent.CanvasCancel);
    });

    nodePopup.emit(ControlEvent.PopupWillAppear);
    ...
    if (animated) {
      ...
      cc.tween(uiOpacity)
        ...
        .call(() => {
          nodePopup.emit(ControlEvent.PopupDidAppear);
        })
        .start();
    } else {
      nodePopup.emit(ControlEvent.PopupDidAppear);
  ...
  _screenWillAppear(screen: BaseScreen, animated?: boolean) {
    this._animating = true;
    screen.node.emit(ControlEvent.ScreenWillAppear, screen);
  }

  _screenDidAppear(screen: BaseScreen, animated?: boolean) {
    this._animating = false;
    screen.node.emit(ControlEvent.ScreenDidAppear, screen);
  }

  _screenWillDisappear(screen: BaseScreen, animated?: boolean) {
    this._animating = true;
    screen.node.emit(ControlEvent.ScreenWillDisappear, screen);
  }

  _screenDidDisappear(screen: BaseScreen, animated?: boolean) {
    this._animating = false;
    screen.node.emit(ControlEvent.ScreenDidDisappear, screen);
  }
