...
export interface ScrollViewDelegate {
  scrollViewDidScroll?(scrollView: ScrollView): void;
  // called on start of dragging (may require some time and or distance to move)
  scrollViewWillBeginDragging?(scrollView: ScrollView): void;
  // called on finger up if the user dragged. velocity is in points/millisecond.
  scrollViewWillEndDragging?(scrollView: ScrollView, velocity: cc.Vec2): void;
  // called on finger up if the user dragged. decelerate is true if it will continue moving afterwards
  scrollViewDidEndDragging?(scrollView: ScrollView, decelerate: boolean): void;

  scrollViewChildrenShouldDrag?(scrollView: ScrollView, event: cc.EventTouch): boolean;
}

export interface ScrollViewDisabledRefObject {
  scrollViewShouldScroll?(scrollView: ScrollView): boolean;
}

@ccclass
export default class ScrollView extends ViewGroup {
  ...
  private _disabledRefObjects: ScrollViewDisabledRefObject[] = [];

  public retainDisabledRef(obj: ScrollViewDisabledRefObject) {
    if (this._disabledRefObjects.indexOf(obj) < 0) {
      this._disabledRefObjects.push(obj);
    }
  }

  public releaseDisabledRef(obj: ScrollViewDisabledRefObject) {
  ...
  public get delegate(): ScrollViewDelegate {
    return this._delegate;
  }

  public set delegate(value: ScrollViewDelegate) {
    this._delegate = value;
  }
  protected _delegate: ScrollViewDelegate = null!;
  ...
  onDisable() {
    ...
    // }
    // cc.Canvas.instance.node.off(ControlEvent.TouchRemoved, this.onTouchRemoved, this);
  }
  ...
  protected _recalculateBoundary() {
    ...
    this._boundaryRect = cc.rect(
      newContentSize.width * -this.node.getComponent(cc.UITransform)!.anchorX,
  ...
  public scrollToLeftTop(animated: boolean = true): boolean {
    return this.scrollTo(
      cc.v2(
        ...
        this.content.getComponent(cc.UITransform)!.height * (1.0 - this.content.getComponent(cc.UITransform)!.anchorY)
      ),
      animated
    );
  }
  ...
  public scrollToLeftBottom(animated: boolean = true): boolean {
    return this.scrollTo(
      cc.v2(
        -this.content.getComponent(cc.UITransform)!.anchorX * this.content.getComponent(cc.UITransform)!.width,
        -this.content.getComponent(cc.UITransform)!.height * this.content.getComponent(cc.UITransform)!.anchorY
      ),
      animated
    );
  }
  ...
  public scrollToRightTop(animated: boolean = true): boolean {
    return this.scrollTo(
      cc.v2(
        (1.0 - this.content.getComponent(cc.UITransform)!.anchorX) * this.content.getComponent(cc.UITransform)!.width,
        this.content.getComponent(cc.UITransform)!.height * (1.0 - this.content.getComponent(cc.UITransform)!.anchorY)
      ),
      animated
    );
  ...
  public scrollToTop(animated: boolean = true): boolean {
    return this.scrollTo(
      cc.v2(
        (0.5 - this.content.getComponent(cc.UITransform)!.anchorX) * this.content.getComponent(cc.UITransform)!.width,
        this.content.getComponent(cc.UITransform)!.height * (1.0 - this.content.getComponent(cc.UITransform)!.anchorY)
      ),
      animated
    );
  ...
  public scrollToLeft(animated: boolean = true): boolean {
    return this.scrollTo(
      cc.v2(
        -this.content.getComponent(cc.UITransform)!.anchorX * this.content.getComponent(cc.UITransform)!.width,
        (0.5 - this.content.getComponent(cc.UITransform)!.anchorY) * this.content.getComponent(cc.UITransform)!.height
      ),
      animated
    );
  }
  ...
  scrollToIn(pos: cc.Vec2, t: number) {
    if ((void 0 === t && (t = 0.5), t <= 0)) return this.setContentPosition(cc.v2(-pos.x * this.content.getScale().x, -pos.y * this.content.getScale().y), false);
  ...
  protected _reAlignBoundIfNeed() {
    ...
    if (this._boundaryRect.x != 0 || this._boundaryRect.y != 0 || this._boundaryRect.width != 0 || this._boundaryRect.height != 0) {
      ...
      let maxX =
        this._boundaryRect.width * this.content.getComponent(cc.UITransform)!.anchorX * this.content.getScale().x -
      ...
      let maxY =
        this._boundaryRect.height * this.content.getComponent(cc.UITransform)!.anchorY * this.content.getScale().y -
      ...
      let minX = -(
        this._boundaryRect.width * (1.0 - this.content.getComponent(cc.UITransform)!.anchorX) * this.content.getScale().x -
      ...
      let minY = -(
        this._boundaryRect.height * (1.0 - this.content.getComponent(cc.UITransform)!.anchorY) * this.content.getScale().y -
  ...
  protected update(dt: number) {
    if (this._autoZoomScale) {
      ...
      percentage = ScrollView.quintEaseOut(percentage);
    ...
    if (this._autoScroll) {
      ...
      this._autoScrollPreviousPosition = cc.Vec2.add(
        this._autoScrollPreviousPosition,
        this._autoScrollStartPosition,
        cc.v2(this._autoScrollDeltaPosition.x * percentage, this._autoScrollDeltaPosition.y * percentage)
      );
  ...
  protected _registerEvent() {
    ...
    cc.find("Canvas")?.on(ControlEvent.CanvasCancel, this._onShowGame, this);
  }
  ...
  protected _unregisterEvent() {
    ...
    cc.find("Canvas")?.off(ControlEvent.CanvasCancel, this._onShowGame, this);
  }
