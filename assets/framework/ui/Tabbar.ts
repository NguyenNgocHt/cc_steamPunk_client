...
export default class Tabbar extends cc.Component {
  ...
  onLoad() {
    ...
    for (let child of this.tabbar.children) {
      if (child.active) {
        let com = child.getComponent(TabbarItem) as TabbarItem;
        if (com) {
  ...
  onEnable() {
    for (let item of this._items) {
      item.node.on(ControlEvent.Click, this.onItemClick, this);
    }
  }

  onDisable() {
    for (let item of this._items) {
      item.node.off(ControlEvent.Click, this.onItemClick, this);
    }
  }
  ...
  selectItemAtIndex(idx: number): boolean {
    ...
    if (this._curSelectedIndex != idx) {
      ...
      this.node.emit(ControlEvent.TabbarItemSelected, this._curSelectedIndex);
      return true;
    }
    return false;
  }
