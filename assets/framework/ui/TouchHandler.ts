...
export default class TouchHandler extends cc.Component {
  ...
  protected _onTouchBegan(event: cc.EventTouch): void {
    ...
    node.emit(ControlEvent.TouchDown, touch);
    this._pressed = true;
  ...
  protected _onTouchMoved(event: cc.EventTouch) {
    ...
    if (this._lastHits[touch.__instanceId] != hit) {
      if (hit) {
        this.node.emit(ControlEvent.TouchDragEnter, touch);
        this.node.emit(ControlEvent.TouchDragInside, touch);
      } else {
        this.node.emit(ControlEvent.TouchDragExit, touch);
        this.node.emit(ControlEvent.TouchDragOutside, touch);
      }
  ...
  protected _onTouchEnded(event: cc.EventTouch) {
    ...
    if (this._pressed) {
      ...
      if (!this._longPressed) {
        ...
        this.node.emit(ControlEvent.ClickByTouchHandler, touch);
      } else {
        this._longPressed = false;
      }
    }

    this.node.emit(ControlEvent.TouchUpInside, touch);
  ...
  protected _onTouchCancelled(event: cc.EventTouch) {
    ...
    this._longPressed = false;
    let touch = event.touch as any;
    this.node.emit(ControlEvent.TouchUpOutside, touch);
    delete this._lastHits[touch.__instanceId];
    this.unscheduleAllCallbacks();
  }
  ...
  private _longClickExec(dt: number) {
    if (this.longClickEnabled && this._pressed && !this._longPressed) {
      this.node.emit(ControlEvent.LongClick, this);
