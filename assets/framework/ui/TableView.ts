...
import ScrollView, { ScrollViewDelegate, VDScrollDirection } from "./ScrollView";
import TableCell from "./TableCell";

export enum TableViewFillOrder {
  LEFT_TO_RIGHT__TOP_TO_BOTTOM = 0,
  TOP_TO_BOTTOM__LEFT_TO_RIGHT = 1,
}

export enum TableViewInteractionMode {
  NONE,
  CLICK,
  SINGLE_SELECTION,
  MULTIPLE_SELECTION,
...
export interface TableViewDelegate extends ScrollViewDelegate {
  tableCellClicked?(tableView: TableView, cell: cc.Node, idx: number): void;

  tableCellDidSelected?(tableView: TableView, idx: number): void;

  tableCellWillDeselected?(tableView: TableView, idx: number): void;

  tableCellWillRecycle?(tableView: TableView, cell: cc.Node, idx: number): void;

  tableCellShouldHighlight?(tableView: TableView, idx: number): boolean;

  tableCellDidUnhighlight?(tableView: TableView, idx: number): void;

  tableCellDidHighlight?(tableView: TableView, idx: number): void;

  tableCellShouldSelect?(tableView: TableView, idx: number): boolean;

  tableClickOnSelectedCell?(tableView: TableView, idx: number): void;
}

export interface TableViewDataSource {
  numberOfCellsInTableView(tableView: TableView): number;
  tableCellAtIndex(tableView: TableView, idx: number): cc.Node;
  tableCellSize?(tableView: TableView): cc.Size;
  tableCellAnchor?(tableView: TableView): cc.Vec2;
}

@ccclass
export default class TableView extends ScrollView {
  @property({
    type: cc.Enum(TableViewFillOrder),
  })
  private fillOrder: TableViewFillOrder = TableViewFillOrder.LEFT_TO_RIGHT__TOP_TO_BOTTOM;

  @property({
    type: cc.Enum(TableViewInteractionMode),
  })
  private interactionMode: TableViewInteractionMode = TableViewInteractionMode.NONE;
  ...
  public get delegate(): TableViewDelegate {
    return this._tableViewDelegate;
  }

  public set delegate(value: TableViewDelegate) {
    this._tableViewDelegate = value;
  }

  private _tableViewDelegate: TableViewDelegate = null!;

  public get dataSource(): TableViewDataSource {
    return this._dataSource;
  }

  public set dataSource(value: TableViewDataSource) {
  ...
  onLoad() {
    ...
    let cache = TableView._cellPoolCache[cacheKey];
    if (!cache) {
      cache = TableView._cellPoolCache[cacheKey] = new cc.NodePool(cacheKey + "<TableCellPool>");
    }
    ...
    if (this._dataSource && this._dataSource.tableCellSize) {
      ...
      this._cellSize = cc.size(
        this.tableCell.data.getComponent(cc.UITransform)!.width * this.tableCell.data.getScale().x,
  ...
  onEnable() {
    super.onEnable();
    if (this.interactionMode != TableViewInteractionMode.NONE) {
  ...
  private _setSelectedForCell(cell: cc.Node, selected: boolean, forceSelected: boolean = false) {
    let cellComp = cell.getComponent(TableCell) as TableCell;
  ...
  public selectCellAtIndex(index: number) {
    ...
    if (this.interactionMode == TableViewInteractionMode.NONE || this.interactionMode == TableViewInteractionMode.CLICK || this._selectedIndices.indexOf(index) >= 0) return;
    ...
    if (!this._tableViewDelegate || !this._tableViewDelegate.tableCellShouldSelect || this._tableViewDelegate.tableCellShouldSelect(this, index)) {
      if (this.interactionMode == TableViewInteractionMode.SINGLE_SELECTION) {
  ...
  public deselectCellAtIndex(index: number) {
    if (this.interactionMode == TableViewInteractionMode.NONE || this.interactionMode == TableViewInteractionMode.CLICK) return;
  ...
  private _onContentClick(touch: cc.Touch) {
    ...
    if (this.interactionMode != TableViewInteractionMode.NONE) {
      ...
      let cell = this.cellAtIndex(index);
      if (cell) {
        if (this.interactionMode == TableViewInteractionMode.CLICK) {
        ...
        } else if (this.interactionMode == TableViewInteractionMode.SINGLE_SELECTION) {
          if (this._selectedIndices.length == 0) {
            if (!this._tableViewDelegate || !this._tableViewDelegate.tableCellShouldSelect || this._tableViewDelegate.tableCellShouldSelect(this, index)) {
              this._selectedIndices.push(index);
              this._setSelectedForCell(cell, true);
        ...
        } else if (this.interactionMode == TableViewInteractionMode.MULTIPLE_SELECTION) {
          if (this._selectedIndices.length == 0) {
            if (!this._tableViewDelegate || !this._tableViewDelegate.tableCellShouldSelect || this._tableViewDelegate.tableCellShouldSelect(this, index)) {
              this._selectedIndices.push(index);
              this._setSelectedForCell(cell, true);
  ...
  public getEndIndex(): number {
    ...
    let endIdx = this._indexFromOffset(
      cc.v2(Math.abs(this.offsetMinX) + this.node.getComponent(cc.UITransform)!.width - dX, Math.abs(this.offsetMaxY) + this.node.getComponent(cc.UITransform)!.height - dY)
    );
  ...
  public scrollToIndex(index: number, animated: boolean = true) {
    ...
    let x =
      -this.content.getComponent(cc.UITransform)!.anchorX * this.content.getComponent(cc.UITransform)!.width +
      offset.x +
      this.node.getComponent(cc.UITransform)!.anchorX * this.node.getComponent(cc.UITransform)!.width;
    let y =
      (1.0 - this.content.getComponent(cc.UITransform)!.anchorY) * this.content.getComponent(cc.UITransform)!.height -
      offset.y -
      (1.0 - this.node.getComponent(cc.UITransform)!.anchorY) * this.node.getComponent(cc.UITransform)!.height;
  ...
  protected scrollViewDidScroll(scrollView: ScrollView) {
  ...
  updateCellAtIndex(idx: number) {
    ...
    if (idx >= this._startIdx && idx <= this._endIdx) {
      ...
      switch (this.interactionMode) {
        case TableViewInteractionMode.SINGLE_SELECTION:
        case TableViewInteractionMode.MULTIPLE_SELECTION:
  ...
  private _indexFromOffset(offset: cc.Vec2) {
    ...
    if (this.direction == VDScrollDirection.VERTICAL) {
      ...
      if (this.fillOrder == TableViewFillOrder.LEFT_TO_RIGHT__TOP_TO_BOTTOM) {
        if (idxX >= this._cols) {
          return -1;
        }
  ...
  private _updateContentSize() {
    ...
    this.content.getComponent(cc.UITransform)!.contentSize.equals(this._realContentSize) ||
      (this.content.getComponent(cc.UITransform)!.setContentSize(this._realContentSize), this.enabledInHierarchy || this._recalculateBoundary());
