...
export default class WebSocketClient {
  ...
  connect(host, port, isSsl, userID, listenner, token) {
    ...
    if (port > 0) {
      this._wsUrl = "ws" + (isSsl ? "s" : "") + "://" + host + ":" + port + "/ws/cf";
    } else {
      this._wsUrl = "ws" + (isSsl ? "s" : "") + "://" + host + "/ws/cf";
    }
  ...
  connectWS(url) {
    ...
    cc.log("[WebSocketClient] init websocket: url = " + this._wsUrl);
  ...
  onSocketOpen() {
    ...
    if (this._listener && this._listener.onSocketOpen) {
      if (this._reconnectCount == 0) this._listener.onSocketOpen.call(this._listener);
      else this._listener.onSocketReconnect.call(this._listener);
  ...
  onSocketMessage(event) {
    ...
    if (this._listener && this._listener.onSocketMessage) {
      // cc.log("@@@ [WebSocketClient] onSocketMessage event = " + JSON.stringify(event)
      //     + ', data = ' + event.data);
  ...
  onSocketError() {
    cc.log("@@@ [WebSocketClient] onSocketError");
    if (this._isClosed) return;
  ...
  onSocketClose() {
    cc.log("@@@ [WebSocketClient] onSocketClose");
    if (this._isClosed) {
      this._isClosed = false;
      return;
    }
  ...
  public send(msg: string) {
    ...
    cc.log("@ [WebSocketClient] send msg = " + msg);
    this._ws.send(msg);
    this._lastTime = new Date().getTime();
  }

  public closeSocket() {
    cc.log("@ [WebSocketClient] closeSocket ");
